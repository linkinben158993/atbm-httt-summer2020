...
public class MBusaryEditMed extends JDialog {
					...
					public void actionPerformed(ActionEvent e) {
						if (unchanged_Info(soLuong, moTa)) {
							JOptionPane.showMessageDialog(null,
									"Không thay đổi thông tin gì cả, quay trở lại trang trước!");
							dispose();
						} else {
							Connection conn = null;
							try {
								conn = new ConnectionControl().createConnection(user.getUserName(), user.getPassword());
								PreparedStatement stmt = conn.prepareStatement("UPDATE QLBV.TOATHUOC "
										+ "SET SOLUONG = ?, MOTA = ?" + " WHERE MAKB = ? AND MAT = ?");
								stmt.setInt(1, Integer.parseInt(comboBox_SoLuong.getSelectedItem().toString()));
								stmt.setString(2, txtMoTa.getText());
								stmt.setString(3, maKB);
						...
						genericStuff.call_revapaint(prevFrame.getTbl_Meds());
					}
				});
	...
	private boolean unchanged_Info(String soLuong, String moTa) {
		if (txtMoTa.getText().equals(moTa) && comboBox_SoLuong.getSelectedItem().toString().equals(soLuong)) {
			return true;
		}
		return false;
	}
