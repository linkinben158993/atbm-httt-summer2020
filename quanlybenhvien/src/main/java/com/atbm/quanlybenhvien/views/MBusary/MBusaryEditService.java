...
public class MBusaryEditService extends JDialog {
	...
	public static void main(String[] args) {
		try {
			MBusaryEditService dialog = new MBusaryEditService(new User(), new String(), new String(),
					new String(), new MBusaryService(new User()));
	...
	public MBusaryEditService(final User user, final String maKB, final String maDV, final String moTa,
			final MBusaryService prevFrame) {
		this.user = user;
		...
		}
		{
			...
			comboBox_MaKB.setEnabled(false);
		...
		}
		{
			...
			comboBox_MaDV.setEnabled(false);
						...
						if (unchanged_Info(maKB, maDV, moTa)) {
							...
							try {
								stmt = conn.prepareStatement(
										"UPDATE QLBV.DIEUPHOIDICHVU SET MAKB = ?, MADV = ?, MOTA = ? WHERE MAKB = ? AND MADV = ?");
								...
								stmt.setString(4, comboBox_MaKB.getSelectedItem().toString());
								stmt.setString(5, comboBox_MaDV.getSelectedItem().toString());

								stmt.executeUpdate();

								conn.close();

								JOptionPane.showMessageDialog(null, "Cập nhật thông tin điều phối thành công!");

							} catch (Exception e2) {
								e2.printStackTrace();
								JOptionPane.showMessageDialog(null, "Cập nhật thông tin điều phối thất bại!");
							}
