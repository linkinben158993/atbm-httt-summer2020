...
public class ReceptionistEditPatients extends JDialog {
	...
	public ReceptionistEditPatients(final String maBN, final User user, final ReceptionistPatients prevFrame) {
		this.user = user;
		this.prevFrame = prevFrame;
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							PreparedStatement stmt = conn.prepareStatement("UPDATE QLBV.BENHNHAN " + "SET HOTEN = ? "
									+ ", NAMSINH = ? " + ", DIACHI = ? " + ", DIENTHOAI = ? " + "WHERE MABN = ?");
	...
	private void fill_Form(String maBN) {
		Connection conn = null;
		try {
			conn = new ConnectionControl().createConnection(getUser().getUserName(), getUser().getPassword());
