...
public class ReceptionistDashBoard extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ReceptionistDashBoard frame = new ReceptionistDashBoard(new User());
	...
	public ReceptionistDashBoard(final User user) {
		...
		ImageIcon imageIcon_Medicines = new ImageIcon(
				ReceptionistDashBoard.class.getResource("/images/Receptionist.png"));
		...
		JLabel lblMnHnhChnh = new JLabel("Màn Hình Chính Tiếp Tân");
		...
		JLabel lblPatients = new JLabel("<html>Bệnh Nhân</html>", SwingConstants.CENTER);
		...
		ImageIcon imageIcon_Patients = new ImageIcon(ReceptionistDashBoard.class.getResource("/images/Patients.png"));
		...
		lblIconPatients.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				dispose();
				ReceptionistPatients receptionistPatients = new ReceptionistPatients(user);
				genericStuff.call_frame(receptionistPatients);
			}
		});
		...
		panelServices.setBackground(Color.LIGHT_GRAY);
		panelServices.setBounds(130, 191, 100, 116);
		panel.add(panelServices);
		JLabel lblServices = new JLabel("<html><center>Điều Phối<br>Dịch Vụ</center></html>", SwingConstants.CENTER);
		lblServices.setForeground(Color.BLACK);
		lblServices.setFont(new Font("Times New Roman", Font.BOLD, 14));
		lblServices.setBounds(0, 86, 100, 30);
		panelServices.add(lblServices);
		JLabel lblIconServices = new JLabel("");
		lblIconServices.setHorizontalAlignment(SwingConstants.CENTER);
		ImageIcon imageIcon_Services = new ImageIcon(ReceptionistDashBoard.class.getResource("/images/Service.png"));
		Image image_Services = imageIcon_Services.getImage();
		Image newImage_Services = image_Services.getScaledInstance(70, 70, java.awt.Image.SCALE_SMOOTH);
		lblIconServices.setIcon(new ImageIcon(newImage_Services));
		lblIconServices.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				dispose();
				ReceptionistService receptionistService = new ReceptionistService(user);
				genericStuff.call_frame(receptionistService);
			}
		});
		genericStuff.hover(lblIconServices, lblServices, panelServices, new Color(230, 230, 250), Color.DARK_GRAY,
				Color.BLACK, Color.LIGHT_GRAY);
