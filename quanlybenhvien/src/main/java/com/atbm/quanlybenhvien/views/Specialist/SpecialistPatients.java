...
public class SpecialistPatients extends JFrame {
	...
	public SpecialistPatients(final User user) {
		...
		getContentPane().add(buttonPane, BorderLayout.LINE_END);
		{
			...
			btnThemBN.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					SpecialistAddPatients specialistAddPatients = new SpecialistAddPatients(user, currFrame);
					genericStuff.call_dialog(specialistAddPatients);
				}
			});
			...
			btnSuaBN.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (tbl_Patients.getSelectedRow() == -1) {
						...
						SpecialistEditPatient specialistEditPatient = new SpecialistEditPatient(user,
								...
								tbl_Patients.getValueAt(tbl_Patients.getSelectedRow(), 7).toString(), currFrame);
						genericStuff.call_dialog(specialistEditPatient);
					}
				}
			});
				...
				public void actionPerformed(ActionEvent e) {
					if (tbl_Patients.getSelectedRow() == -1) {
						...
						try {
							...
							PreparedStatement stmt = conn.prepareStatement("DELETE FROM QLBV.BENHNHAN WHERE MABN = ?");
		...
		label_13.setBounds(50, 438, 614, 14);
