...
public class ReceptionistAddServices extends JDialog {
	...
	private JComboBox<String> comboBox_KB;
	...
	public ReceptionistAddServices(final User user, final ReceptionistService prevFrame) {
		...
		comboBox_KB = new JComboBox<String>();
		comboBox_KB.setBounds(120, 11, 150, 20);
		Connection connKB = null;
		try {
			connKB = new ConnectionControl().createConnection(user.getUserName(), user.getPassword());
			PreparedStatement stmt = connKB.prepareStatement("SELECT * FROM QLBV.KHAMBENH");
			ResultSet res = stmt.executeQuery();
			if (res.next() == false) {
				JOptionPane.showMessageDialog(null, "Chưa có khám bệnh nào!");
			} else {
				do {
					comboBox_KB.addItem(res.getString("MAKB"));
				} while (res.next());
			}
			connKB.close();
		...
		comboBox_DichVu.setBounds(120, 39, 150, 20);
		Connection conn_DV = null;
		try {
			conn_DV = new ConnectionControl().createConnection(user.getUserName(), user.getPassword());
			PreparedStatement stmt = conn_DV.prepareStatement("SELECT * FROM QLBV.DICHVU");
			ResultSet res = stmt.executeQuery();
			if (res.next() == false) {
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							stmt.setString(1, comboBox_KB.getSelectedItem().toString());
							...
							JOptionPane.showMessageDialog(null, "Điều phối dịch vụ thành công!");
							...
							JOptionPane.showMessageDialog(null, "Điều phối dịch vụ thất bại!");
						}

						dispose();
						prevFrame.getTableDieuPhoi().fireTableDataChanged();
						prevFrame.draw_TableDieuPhoi();
						prevFrame.getTblDieuPhoi().setModel(prevFrame.getTableDieuPhoi());
						genericStuff.resizeTable(prevFrame.getTblDieuPhoi());
