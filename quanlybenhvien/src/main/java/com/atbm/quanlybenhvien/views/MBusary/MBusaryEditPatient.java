				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (unchanged_Info(BN, Phong, BS, moTa)) {
							...
							try {
								conn = new ConnectionControl().createConnection(user.getUserName(), user.getPassword());
								PreparedStatement stmt = conn.prepareStatement("UPDATE QLBV.KHAMBENH "
										+ "SET BENHNHAN = ?, PHONG = ?, BACSI = ?, MOTA = ?" + " WHERE MAKB = ?");
								stmt.setString(1, comboBox_BN.getSelectedItem().toString());
								stmt.setString(2, comboBox_Phong.getSelectedItem().toString());
								stmt.setString(3, comboBox_BS.getSelectedItem().toString());
								stmt.setString(4, txtMoTa.getText());
								stmt.setString(5, maKB);
								stmt.executeUpdate();
								conn.close();
