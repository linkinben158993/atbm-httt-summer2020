			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (comboBox_NV.getSelectedItem() == null || comboBox_Phong.getSelectedItem() == null
								|| comboBox_SoNgay.getSelectedItem() == null) {
							JOptionPane.showMessageDialog(null, "Vui lòng chọn đầy đủ các mục khi thêm!");
						} else {
							SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yy");
							Date dateStart_comp = null;
							try {
								dateStart_comp = sdf.parse(sdf.format(getDatePickerModel_Start().getValue()).toString());
							...
							try {
								stmt = conn.prepareStatement("INSERT INTO QLBV.TRUCPHONG VALUES (?,?,?,?)");
								stmt.setString(1, comboBox_Phong.getSelectedItem().toString());
								stmt.setString(2, comboBox_NV.getSelectedItem().toString());
								stmt.setTimestamp(3, time_Tu);
								stmt.setTimestamp(4, time_Den);
								stmt.executeUpdate();
								conn.close();

								JOptionPane.showMessageDialog(null, "Thêm mới trực phòng thành công!");

							} catch (SQLException e1) {
								e1.printStackTrace();
								JOptionPane.showMessageDialog(null, "Thêm mới trực phòng thất bại!");
								try {
									conn.close();
